...
def get_args():
    ...
    ap.add_argument("-d", "--data", type=str, required=False,
                    default="../Datasets/", help="Datasets path")
...
def create_subset(f, im_paths, annotations, bsize, joint_names, visualize):
    ...
    for i, path in enumerate(im_paths):
        ...
        if dataset == "lsp":
            ...
            # Store human data in created subet
            data = np.array(("lsp", im_name, scale, center, joints),
                            dtype=labels_type)
            labels[n_humans] = data
            images[n_humans] = im_human
...
if __name__ == "__main__":
    ...
    all_labels = (mpii_labels, lsp_labels)
    ...
    ftrain = h5py.File("../Datasets/mpii_lsp_train.h5", mode="w")
    fval = h5py.File("../Datasets/mpii_lsp_val.h5", mode="w")
    ftest = h5py.File("../Datasets/mpii_lsp_test.h5", mode="w")
