...
"""
parse_head_anno.py: Parse XMLs containing head bounding box for each
LSP image.
"""
...
if __name__ == "__main__":
    ...
    for xml_anno in glob.glob(path + "*.xml"):
        with open(xml_anno) as f:
            ...
            if not xml_dict["annotation"]["object"]["deleted"]:
                head_bbox = xml_dict["annotation"]["object"]["polygon"]["pt"]
                x1, y1 = head_bbox[0].values()
                x2, y2 = head_bbox[2].values()

                new_anno[str(im_id)] = [int(x1), int(y1), int(x2), int(y2)]

    np.save("head_anno.npy", new_anno)
